...
const Billing = () => {
   ...
   const generatePDF = (data) => {
      ...
      // Header
      doc.setFontSize(22);
      doc.text(data.shopName, 105, 20, { align: "center" });
      doc.setFontSize(12);
      doc.text(data.invoiceNo === "DRAFT" ? "DRAFT TAX INVOICE" : "TAX INVOICE", 105, 30, { align: "center" });
      
      // Details
      doc.setFontSize(10);
      doc.text(`Billed To: ${data.customerName}`, 14, 45);
      doc.text(`WhatsApp: ${data.whatsapp !== "N/A" ? "+91 " + data.whatsapp : data.whatsapp}`, 14, 52);
      
      doc.text(`Invoice No: ${data.invoiceNo}`, 140, 45);
      doc.text(`Date: ${data.date}`, 140, 52);
      ...
      // Totals
      doc.text(`Subtotal: Rs. ${data.subtotal.toFixed(2)}`, 140, finalY + 10);
      doc.text(`GST (18%): Rs. ${data.gst.toFixed(2)}`, 140, finalY + 17);
      doc.setFont("helvetica", "bold");
      doc.text(`Grand Total: Rs. ${data.total.toFixed(2)}`, 140, finalY + 25);
      doc.setFont("helvetica", "normal");
      
      // Footer
      doc.text(`Generated by ${data.billerName}`, 105, finalY + 40, { align: "center" });
      doc.text("Thank you for your business!", 105, finalY + 47, { align: "center" });
