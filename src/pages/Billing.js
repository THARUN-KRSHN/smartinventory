...
const Billing = () => {
      ...
      const fetchShopMeta = async () => {
         try {
            ...
            if (shopId && shopId !== "null") {
               ...
               if (shop && shop.logo && isMounted) setShopLogo(shop.logo);
            }
         } catch (e) {}
      }
   ...
   const handleAddFromCard = (product) => {
      ...
      if (existingItem) {
         if (product.quantity && parseInt(existingItem.quantity) + 1 > product.quantity) {
         ...
         if (emptyRow) {
            setOrderItems(orderItems.map(item => {
               if (item.id === emptyRow.id) {
                  return { ...item, product_id: product.product_id ?? product.id, product_name: product.product_name, price: product.price, quantity: 1, available_quantity: product.quantity };
               }
               return item;
            }));
         } else {
         ...
         if (item.id === id) {
            ...
            if (field === "product_id" && value) {
               ...
               if (selectedProduct) {
                  ...
                  updatedItem.available_quantity = selectedProduct.quantity;
               }
               updatedItem.product_id = value;
            } else if (field === "quantity") {
               let numValue = value === '' ? '' : parseInt(value);
               if (updatedItem.available_quantity !== undefined && typeof numValue === "number" && numValue > updatedItem.available_quantity) {
                  alert(`Insufficient stock! Only ${updatedItem.available_quantity} available.`);
                  numValue = updatedItem.available_quantity;
               }
   ...
   const generatePDF = async (data) => {
      ...
      const printHeight = Math.max(140, 100 + (data.items.length * 9));
      const doc = new jsPDF({
      ...
      if (data.shopLogo) {
         const base64Logo = await getBase64ImageFromUrl(data.shopLogo);
         if (base64Logo) {
      ...
      doc.setFontSize(9);
      doc.setFont("helvetica", "bold");
      doc.text(data.shopName.substring(0, 30), pageWidth / 2, currentY, { align: "center" });
      currentY += 6;

      doc.setFont("helvetica", "normal");
      doc.setFontSize(8);
      doc.text(`Date: ${data.date.split(',')[0]}`, 5, currentY);
      currentY += 5;
      doc.text(`Manager: ${data.billerName}`, 5, currentY);
      currentY += 6;
      
      if (data.customerName && data.customerName !== "Walk-in Customer") {
         doc.text(`Customer: ${data.customerName}`, 5, currentY);
         currentY += 5;
      }
      if (data.whatsapp && data.whatsapp !== "N/A") {
         doc.text(`Phone: +91 ${data.whatsapp}`, 5, currentY);
         currentY += 5;
      }
      ...
      doc.text(data.subtotal.toFixed(2), 75, currentY, { align: "right" });
      currentY += 5;
      
      doc.text("Tax (18%):", 35, currentY);
      doc.text(data.gst.toFixed(2), 75, currentY, { align: "right" });
      currentY += 5;
      
      doc.setFontSize(10);
      doc.setFont("helvetica", "bold");
      doc.text("Total:", 35, currentY);
      doc.text(`Rs. ${data.total.toFixed(2)}`, 75, currentY, { align: "right" });
      currentY += 10;
      ...
      doc.setFontSize(7);
      doc.setFont("helvetica", "normal");
      doc.text(`#${data.invoiceNo}`, pageWidth / 2, currentY, { align: "center" });
      currentY += 8;
   ...
   const handleGenerateInvoice = async () => {
      ...
      try {
         ...
         // Avoid calling .padStart on a full uuid string directly if it's alphanumeric. If sale_id is an Int, padStart is fine.
         // Wait, sale_id is a UUID, padStart on it is weird. Let's just use the first chunk of UUID.
         const safeSaleIdStr = response.sale_id ? response.sale_id.toString().split('-')[0] : "";

         const generatedInvoiceData = {
            invoiceNo: response.sale_id ? `INV-${safeSaleIdStr.padStart(6, '0')}` : `INV-${Math.floor(100000 + Math.random() * 900000)}`,
            date: new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }),
            customerName: customerName || "Walk-in Customer",
            whatsapp: whatsapp || "N/A",
            items: validItems,
         ...
         // Auto-generate PDF
         generatePDF(generatedInvoiceData);
   ...
   return (
      <div className="row h-100 g-0">
                        ...
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, scale: 0.95 }} key={item.id} className="row g-2 align-items-center">
                           ...
                           <div className="col-3">
                              <div className="d-flex align-items-center bg-white border rounded-3 overflow-hidden">
                                 ...
                                 <input type="text" className="form-control bg-transparent border-0 px-1 py-1 text-center shadow-none fw-bold text-dark" value={item.quantity === '' ? '' : item.quantity} onChange={(e) => handleItemChange(item.id, "quantity", e.target.value.replace(/[^0-9]/g, ''))} style={{ WebkitAppearance: 'none', MozAppearance: 'textfield' }} />
         ...
         {showInvoice && invoiceData && (
            <div 
               className="position-fixed top-0 start-0 w-100 h-100 z-3 bg-dark bg-opacity-75 d-flex align-items-center justify-content-center py-4 px-3 p-md-5 overflow-auto"
               onClick={(e) => {
